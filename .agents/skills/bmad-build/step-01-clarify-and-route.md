...
3. Otherwise — scan artifacts and ask
   - Active specs (`draft`, `ready-for-dev`, `in-progress`, `in-review`) in `{{config.modules.bmm.implementation_artifacts}}`? → List them and HALT. Ask user which to resume (or `[N]` for new).
...
4. Multi-goal check (see SCOPE STANDARD). If the intent fails the single-goal criteria:
   ...
   - On **S**: For each deferred goal, append one new entry to `{{config.modules.bmm.implementation_artifacts}}/deferred-work.md` using this format. Do not modify existing entries or look for duplicates. Narrow scope to the first-mentioned goal. Continue routing.
     ```markdown
     - source_spec: none
