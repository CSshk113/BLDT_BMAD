...
export default function ApplicationsPage() {
  ...
  const refresh = async (selectedId?: string) => {
    ...
    setCatalog(result);
    const nextId = selectedId ?? selected?.id ?? result.items[0]?.id;
    if (nextId) setSelected(await getApplication(nextId));
  };
  ...
  const select = (id: string) => getApplication(id).then(setSelected).catch(() => setError("지원서 상세 정보를 불러오지 못했습니다."));
  const uploaded = (application: ApplicationDetail) => { setSelected(application); refresh(application.id).catch(() => setError("업로드 후 목록을 갱신하지 못했습니다.")); };
  const retry = () => selected && reprocessApplication(selected.id).then((application) => { setSelected(application); return refresh(application.id); }).catch(() => setError("재처리를 시작하지 못했습니다."));
  ...
  return <main className="mx-auto grid max-w-7xl gap-6 p-6" aria-labelledby="applications-heading">
    ...
    {error && <Alert variant="destructive"><AlertTitle>서버 연결 오류</AlertTitle><AlertDescription>{error}</AlertDescription></Alert>}
